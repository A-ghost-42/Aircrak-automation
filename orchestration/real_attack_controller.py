...
class RealAttackController:
    ...
    def execute_real_attack_cycle(self, targets, interface='wlan0mon', seeds=None, wordlist=None, user_passwords=None):
        """
        Execute real attack cycle with handshake capture
        """
        print("\n" + "="*60)
        print("REAL ATTACK CYCLE EXECUTION")
        print("="*60)
        ...
        for target in targets:
            ...
            if signal_strength < -80:
                print("   Skipping - signal too weak")
                continue
            
            result = self.real_attack_engine.execute_real_attack(
                target, interface, seeds=seeds, wordlist=wordlist, user_passwords=user_passwords,
            )
